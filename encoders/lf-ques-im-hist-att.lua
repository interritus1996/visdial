...
function encoderNet.model(params)
    ...
    local mm = nn.MM(){img_feats_view,q3_view}
    local prob = nn.SoftMax()(nn.View(-1,196)(mm))
    local prob_view = nn.View(-1,196,1)(prob)

    local mm1 = nn.MM(true,false){prob_view,img_feats_view}
    local mm1_view = nn.View(-1,512)(mm1)

    local q_join = nn.JoinTable(1,1)({mm1_view,q3,h3})
    local final_layer = nn.Tanh()(nn.Linear(1536,512)(nn.Dropout(dropout)(q_join)))
